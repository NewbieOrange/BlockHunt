            ...
            LocationSerializable location)
    {
        if (lines[1] != null)
        {
            ...
            else
            {
                ...
                for (Arena arena : W.arenaList)
                {
                    if (lines[1].equals(arena.arenaName))
                    {
                        ...
                        while (!saved)
                        {
                            ...
                            {
                                W.signs.getFile().set(
                                        arena.arenaName + "_" + number + ".arenaName",
                                        lines[1]);
                                W.signs.getFile().set(
                                        arena.arenaName + "_" + number + ".location",
                                        location);
                                W.signs.save();
                ...
                if (!saved)
                {
                    MessageM.sendFMessage(event.getPlayer(), ConfigC.error_noArena,
                            "name-" + lines[1]);
                }
            }
        }
    ...
    public static void removeSign(LocationSerializable location)
    {
        for (String sign : W.signs.getFile().getKeys(false))
        {
            LocationSerializable loc = new LocationSerializable(
                    (LocationSerializable) W.signs.getFile().get(sign + ".location"));
    ...
    public static void updateSigns()
    {
        ...
        for (String sign : W.signs.getFile().getKeys(false))
        {
                    ...
                    || loc.getBlock().getType().equals(Material.WALL_SIGN))
            {
                ...
                {
                    ArrayList<String> signLines = (ArrayList<String>) W.config.getFile()
                            .getList(ConfigC.sign_LEAVE.location);
                    int linecount = 0;
                ...
                {
                    ArrayList<String> signLines = (ArrayList<String>) W.config.getFile()
                            .getList(ConfigC.sign_SHOP.location);
                    int linecount = 0;
                    for (String line : signLines)
                    {
                        if (linecount <= 3)
                        ...
                        if (lines[1].endsWith(arena.arenaName))
                        {
                            if (arena.gameState.equals(ArenaState.WAITING))
                            {
                                ArrayList<String> signLines = (ArrayList<String>) W.config
                                        .getFile().getList(ConfigC.sign_WAITING.location);
                                int linecount = 0;
                                if (signLines != null)
                                {
                                    for (String line : signLines)
                            ...
                            else if (arena.gameState.equals(ArenaState.STARTING))
                            {
                                ArrayList<String> signLines = (ArrayList<String>) W.config
                                        .getFile()
                                        .getList(ConfigC.sign_STARTING.location);
                                int linecount = 0;
                                if (signLines != null)
                                {
                                    for (String line : signLines)
                            ...
                            {
                                ArrayList<String> signLines = (ArrayList<String>) W.config
                                        .getFile().getList(ConfigC.sign_INGAME.location);
                                int linecount = 0;
                                if (signLines != null)
                                {
                                    for (String line : signLines)
