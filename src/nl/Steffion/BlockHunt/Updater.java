...
public class Updater
{
    ...
    private void saveFile(File folder, String file, String u)
    {
        ...
        try
        {
            ...
            int count;
            if (announce)
                MessageM.sendMessage(null, "%TAG%NAbout to download a new update: "
                        + versionTitle);
            long downloaded = 0;
            while ((count = in.read(data, 0, BYTE_SIZE)) != -1)
            {
                ...
                {
                    MessageM.sendMessage(null, "%TAG%NDownloading update: " + percent
                            + "% of " + fileLength + " bytes.");
                }
            }
        ...
        try
        {
            ...
            while (e.hasMoreElements())
            {
                ...
                else
                {
                    ...
                    {
                        destinationFilePath.renameTo(new File("plugins/" + updateFolder
                                + "/" + name));
                    }
                }
                entry = null;
