...
public class CMDtokens extends DefaultCMD
{
    ...
    public boolean exectue(Player player, Command cmd, String label, String[] args)
    {
        if (args.length <= 3)
        {
            MessageM.sendFMessage(player, ConfigC.error_notEnoughArguments, "syntax-"
                    + BlockHunt.CMDtokens.usage);
        }
        else
        {
            ...
            if (tokenPlayer == null)
            {
                MessageM.sendFMessage(player, ConfigC.error_tokensPlayerNotOnline,
            ...
            if (option.equalsIgnoreCase("set"))
            {
                ...
                W.shop.save();
                MessageM.sendFMessage(player, ConfigC.normal_tokensChanged, "option-Set",
                        "playername-" + tokenPlayer.getName(), "option2-to", "amount-"
                                + amount);
                MessageM.sendFMessage(tokenPlayer, ConfigC.normal_tokensChangedPerson,
                        "option-set", "playername-" + name, "option2-to", "amount-"
                                + amount);
            }
            else if (option.equalsIgnoreCase("add"))
            {
                ...
                MessageM.sendFMessage(tokenPlayer, ConfigC.normal_tokensChangedPerson,
                        "option-added", "playername-" + name, "option2-to", "amount-"
                                + amount);
            }
            else if (option.equalsIgnoreCase("take"))
            {
                ...
                MessageM.sendFMessage(tokenPlayer, ConfigC.normal_tokensChangedPerson,
                        "option-took", "playername-" + name, "option2-from", "amount-"
                                + amount);
            }
            else
            {
                MessageM.sendFMessage(player, ConfigC.error_tokensUnknownsetting,
                        "option-" + option);
            }
        }
        return true;
