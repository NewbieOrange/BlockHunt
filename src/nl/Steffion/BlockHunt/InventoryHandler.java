...
public class InventoryHandler
{
    ...
    public static void openPanel(Player player, String arenaname)
    {
        ...
        if (arena != null)
        {
            ...
            Inventory panel = Bukkit.createInventory(null, 54,
            ...
            arenaNameNote_IM.setDisplayName(MessageM.replaceAll("%NSettings of arena: %A"
                    + arena.arenaName));
            ...
            updownButton(panel, arena, ArenaType.timeInLobbyUntilStart,
                    "timeInLobbyUntilStart", "5 %Nsecond", timeInLobbyUntilStart_UP,
                    timeInLobbyUntilStart, timeInLobbyUntilStart_DOWN, 6, 15, 24);
            
            updownButton(panel, arena, ArenaType.waitingTimeSeeker, "waitingTimeSeeker",
                    "5 %Nsecond", waitingTimeSeeker_UP, waitingTimeSeeker,
                    waitingTimeSeeker_DOWN, 7, 16, 25);
            
            updownButton(panel, arena, ArenaType.gameTime, "gameTime", "10 %Nsecond",
                    gameTime_UP, gameTime, gameTime_DOWN, 8, 17, 26);
            
            updownButton(panel, arena, ArenaType.timeUntilHidersSword,
                    "timeUntilHidersSword", "5 %Nsecond", timeUntilHidersSword_UP,
                    timeUntilHidersSword, timeUntilHidersSword_DOWN, 30, 39, 48);
            
            updownButton(panel, arena, ArenaType.hidersTokenWin, "hidersTokenWin",
                    "1 %Ntoken", hidersTokenWin_UP, hidersTokenWin, hidersTokenWin_DOWN,
                    32, 41, 50);
            
            updownButton(panel, arena, ArenaType.seekersTokenWin, "seekersTokenWin",
                    "1 %Ntoken", seekersTokenWin_UP, seekersTokenWin,
                    seekersTokenWin_DOWN, 33, 42, 51);
            
            updownButton(panel, arena, ArenaType.killTokens, "killTokens", "1 %Ntoken",
                    killTokens_UP, killTokens, killTokens_DOWN, 34, 43, 52);
            ...
            ItemStack DOWN, int up, int button, int down)
    {
        ...
        UP_IM.setDisplayName(MessageM.replaceAll(
                (String) W.messages.get(ConfigC.button_add), "1-" + addremove, "2-"
                        + option));
        ...
        BUTTON_IM.setDisplayName(MessageM.replaceAll(
                (String) W.messages.get(ConfigC.button_setting), "1-" + option, "2-"
                        + setting));
        BUTTON.setItemMeta(BUTTON_IM);
        
        ItemMeta DOWN_IM = DOWN.getItemMeta();
        DOWN_IM.setDisplayName(MessageM.replaceAll(
                (String) W.messages.get(ConfigC.button_remove), "1-" + addremove, "2-"
                        + option));
    ...
    public static void openShop(Player player)
    {
        Inventory shop = Bukkit.createInventory(null, 9,
                MessageM.replaceAll("\u00A7r" + W.config.get(ConfigC.shop_title)));
        if (W.config.getFile().getBoolean("vaultSupport") == true)
        {
            if (BlockHunt.econ != null)
            {
                ...
                shopTokens_IM.setDisplayName(MessageM.replaceAll("%N&lBalance: %A"
                        + vaultBalance));
                ...
                lores2.add(MessageM.replaceAll(
                        (String) W.config.get(ConfigC.shop_vaultPrice), "amount-"
                                + W.config.getFile().getInt("blockChooserPrice")));
        ...
        else
        {
            ...
            lores2.add(MessageM.replaceAll((String) W.config.get(ConfigC.shop_price),
            ...
            shop.setItem(0, shopTokens);
            if ((Boolean) W.config.get(ConfigC.shop_blockChooserv1Enabled) == true
                    && ((Boolean) W.shop.getFile()
                            .get(player.getName() + ".blockchooser") == null && !PermissionsM
                            .hasPerm(player, Permissions.shopblockchooser, false)))
            {
                shop.setItem(1, shopBlockChooser);
            }
